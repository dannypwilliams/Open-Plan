...
For an incremental export, append `--only AssetName` after the project root. Repeat `--only` for multiple assets. The generator retains untouched entries in both manifests.
...
Validation checks the expected 54-asset set, source/FBX presence, non-empty objects, and required hero assets. See [ASSET_MANIFEST.md](ASSET_MANIFEST.md) and `Tools/Blender/Logs`.
